...
- **TOML 자동 주입**: `install.py`가 `~/.codex/config.toml`을 in-place 편집할 때, hook 6종은 marker로 감싼 블록을 파일 끝에 append하고, `[features] hooks = true`는 기존 섹션 내에 한 줄(태그 주석 포함)로 삽입한다. 이중 `[features]` 헤더는 TOML redefinition 에러이므로 절대 넣지 않는다.
